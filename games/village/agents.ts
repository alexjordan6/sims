...
export abstract class Mover implements Agent {
  ...
  tryAttack(target: Mover, dmg: number, reach = 26, cooldown = 0.8): boolean {
...
export class Villager extends Mover {
  ...
  applyRole(): void {
    switch (this.role) {
      case 'kid': this.radius = 4; this.color = 0xf5d8a8; this.maxHp = 10; this.speed = 60; break;
      case 'farmer': this.radius = 6; this.color = 0x7fd37f; this.maxHp = 20; this.speed = 70; break;
      case 'woodcutter': this.radius = 6; this.color = 0xc9a26b; this.maxHp = 20; this.speed = 70; break;
      case 'soldier': this.radius = 6; this.color = 0x6f9bff; this.maxHp = p.soldierHp; this.speed = 90; break;
    }
  ...
  comeOfAge(s: VillageScene): void {
    ...
    this.applyRole();
    this.hp = this.maxHp;
    s.log(`${this.name} grew up: ${this.role} (martial ${this.martial.toFixed(0)} vs civil ${this.civil.toFixed(0)})`);
  }
  ...
  private kidUpdate(dt: number, s: VillageScene): void {
    const danger = s.nearestRaider(this.x, this.y, 160);
    if (danger) { this.martial += 4 * dt; this.goHome(s, dt); return; }
    ...
    s.grid.forEachInRadius(this.x, this.y, 96, (o) => {
  ...
  private civilUpdate(dt: number, s: VillageScene, farmer: boolean): void {
    if (s.nearestRaider(this.x, this.y, 180)) { this.goHome(s, dt); return; }
    ...
    if (!this.goal && this.thinkTimer <= 0) {
      ...
      if (job) this.setGoal(s, job.tx, job.ty);
      else this.wanderNear(s, this.home);
      return;
    }
    ...
    if (this.goal && this.followPath(dt)) {
      ...
      if (isJob && this.adjacentTo(this.goal)) this.workTimer = farmer ? 1.2 : 2.5;
      else this.clearGoal();
    }
  }
  ...
  private soldierUpdate(dt: number, s: VillageScene): void {
    ...
    if (this.retarget <= 0) {
      this.retarget = 0.4;
      this.target = s.nearestRaider(this.x, this.y, 260);
    }
...
export class Player extends Mover {
  ...
  get faced(): TilePos {
    return World.toTile(this.x + this.facing.x * 22, this.y + this.facing.y * 22);
  }
